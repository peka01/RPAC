...
export function PersonalDashboard({ user }: PersonalDashboardProps = {}) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Resource Health Overview - Enhanced with Section Header */}
      <div className="bg-white rounded-xl p-6 shadow-sm border">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--color-muted)' }}>
              Översikt över dina resurser och förberedelser
            </p>
          </div>
          <button className="text-sm flex items-center space-x-1 px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors" style={{ color: 'var(--color-primary)' }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {resourceCategories.map((category, index) => {
            ...
            return (
              <div key={index} className="bg-white rounded-xl p-4 border-2 hover:shadow-md transition-all cursor-pointer group"
                   style={{ 
                     borderColor: isCritical ? `${statusColor}40` : 'var(--color-secondary)20',
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
                        {category.daysRemaining > 0 ? `${category.daysRemaining} dagar kvar` : 'Inga resurser'}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
