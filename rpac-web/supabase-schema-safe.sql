-- RPAC Safe Supabase Database Schema
-- This script safely adds new tables and features without breaking existing data
-- Use this if you already have a Supabase database with content
...
-- Resources table for individual inventory (check if exists first)
DO $$
BEGIN
...
-- =============================================
-- CULTIVATION & PLANNING SYSTEM (NEW TABLES)
-- =============================================
...
-- =============================================
-- PLANT DIAGNOSIS & AI (SAFE CREATION)
-- =============================================

-- Plant diagnoses (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'plant_diagnoses') THEN
...
-- =============================================
-- COMMUNITY & MESSAGING (SAFE CREATION)
-- =============================================

-- Local communities (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'local_communities') THEN
...
-- =============================================
-- RESOURCE SHARING & HELP REQUESTS (SAFE CREATION)
-- =============================================

-- Resource sharing (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'resource_sharing') THEN
...
-- Help requests (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'help_requests') THEN
  ...
  ELSE
    -- Add new columns if they don't exist (only if local_communities table exists)
...
-- =============================================
-- EXTERNAL COMMUNICATION (NEW TABLES)
-- =============================================
...
-- Resources indexes (only if table and columns exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'resources') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'resources' AND column_name = 'is_msb_recommended') THEN
...
-- Community indexes (only if tables exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'local_communities') THEN
...
-- Help requests indexes (only if table and columns exist)
DO $$
BEGIN
...
ALTER TABLE external_communication_sources ENABLE ROW LEVEL SECURITY;
ALTER TABLE external_alerts ENABLE ROW LEVEL SECURITY;
...
-- Community policies (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'local_communities') THEN
...
-- Resource sharing policies (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'resource_sharing') THEN
...
-- Apply updated_at triggers to all relevant tables (safe creation)
...
-- Insert default external communication sources (only if not exists)
...
-- View for user dashboard data (only if user_profiles exists)
DO $$
BEGIN
...
-- View for community overview (only if local_communities exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'local_communities') THEN
...
DO $$
BEGIN
  RAISE NOTICE 'RPAC Safe Database Schema Update Completed Successfully!';
  RAISE NOTICE 'New tables added: user_profiles, cultivation_calendar, garden_layouts, cultivation_reminders, crisis_cultivation_plans, nutrition_calculations, community_memberships, messages, external_communication_sources, external_alerts';
  RAISE NOTICE 'Existing tables preserved and enhanced with new columns where needed.';
  RAISE NOTICE 'Indexes, RLS policies, triggers, and functions configured safely.';
  RAISE NOTICE 'Ready for RPAC application with cultivation features!';
END $$;
