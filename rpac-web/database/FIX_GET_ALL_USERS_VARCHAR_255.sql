-- =============================================
-- FIX get_all_users FUNCTION - VARCHAR(255) FIX
-- =============================================
-- Fixes: Returned type character varying(255) does not match expected type text in column 2
-- The display_name column in the database is VARCHAR(255), not VARCHAR(100) or TEXT
-- Date: 2025-10-22
...
-- Create the correct version with VARCHAR(255) for display_name
...
RETURNS TABLE (
  user_id UUID,
  email TEXT,
  display_name VARCHAR(255),  -- ✅ Changed from VARCHAR(100) to VARCHAR(255)
...
BEGIN
  ...
  SELECT 
    au.id as user_id,
    au.email::TEXT,
    up.display_name,  -- Will return as VARCHAR(255)
...
BEGIN
  ...
  IF v_function_exists THEN
    ...
    RAISE NOTICE '✅ Fixed VARCHAR(255) type mismatch for display_name';
  ELSE
